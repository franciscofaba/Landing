      ...
      user: (
        <div className="flex items-center gap-2">
           ...
           <div className="h-4 w-24 flex items-center gap-0.5">
             {[...Array(12)].map((_,i) => (
                <div key={i} className="w-1 bg-slate-800 rounded-full" style={{height: Math.random() * 16 + 4 + 'px'}}></div>
             ))}
           </div>
        </div>
      ),
...
function SuperpowersSection() {
  ...
  return (
    <section className="min-h-screen bg-slate-950 text-white py-20 px-6 flex flex-col items-center justify-center font-sans selection:bg-purple-500 selection:text-white">
      ...
      <div className="max-w-6xl w-full">
        ...
        <div className="mb-16 md:text-center max-w-2xl mx-auto">
          ...
          <p className="text-slate-400 text-lg">
            Deja de intentar recordarlo todo. Delega tu memoria y recupera tu tranquilidad.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={`mt-1 p-2 rounded-lg transition-colors duration-300 ${isActive ? feature.color : "bg-slate-800 text-slate-400 group-hover:bg-slate-700"}`}>
                      <Icon className={`w-5 h-5 ${isActive ? "text-white" : ""}`} />
                    </div>
                    <div>
                      <h3 className={`text-lg font-bold mb-2 transition-colors ${isActive ? "text-white" : "text-slate-400 group-hover:text-slate-200"}`}>
                        {feature.title}
                      </h3>
          ...
          <div className="lg:col-span-7 relative h-[500px] bg-slate-900 rounded-[40px] border border-slate-800 shadow-2xl flex items-center justify-center p-8 overflow-hidden">
            ...
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 rounded-full blur-[100px] opacity-20 transition-colors duration-700 ${features[activeTab].color}`}></div>
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5"></div>

            {/* Mockup del Chat */}
            <div className="relative w-full max-w-md bg-slate-950 rounded-3xl border border-slate-800 p-6 shadow-xl z-10 transition-all duration-500 transform key={activeTab}">
              
              {/* Header del Chat Falso */}
              <div className="flex items-center gap-3 mb-8 border-b border-slate-900 pb-4">
                 <div className="w-3 h-3 bg-red-500 rounded-full"></div>
                 <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                 <div className="w-3 h-3 bg-green-500 rounded-full"></div>
                 <span className="ml-auto text-xs font-mono text-slate-600">MEMORAE-AI v2.0</span>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-end animate-fade-in-right">
                  <div className={`max-w-[85%] rounded-2xl rounded-tr-sm p-4 text-white text-sm shadow-lg ${features[activeTab].color}`}>
                ...
                <div className="flex justify-start animate-fade-in-left" style={{animationDelay: '150ms'}}>
                   <div className="max-w-[85%] bg-slate-800 rounded-2xl rounded-tl-sm p-4 text-slate-200 text-sm shadow-lg border border-slate-700">
